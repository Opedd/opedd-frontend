...
export function BulkPricingModal({ open, onOpenChange, selectedIds, onSuccess }: BulkPricingModalProps) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (humanPrice !== "") updates.human_price = parseFloat(humanPrice) || 0;
      if (aiPrice !== "") updates.ai_price = parseFloat(aiPrice) || 0;
      updates.licensing_enabled = listOnMarketplace;

      await Promise.all(
        selectedIds.map((id) =>
          supabase.from("assets").update(updates).eq("id", id).eq("user_id", user.id)
        )
      );
