    ...
    return (
      <div className="text-center py-4">
        ...
        <div className="relative w-32 h-32 mx-auto mb-8">
          ...
          {/* Sparkle accents */}
          <div className="absolute top-1 right-1 w-2 h-2 bg-oxford rounded-full animate-ping" style={{ animationDuration: '2s', animationDelay: '0.5s' }} />
          <div className="absolute top-6 right-[-4px] w-1.5 h-1.5 bg-violet-600 rounded-full animate-ping" style={{ animationDuration: '2.5s', animationDelay: '1s' }} />
        </div>

        <h2 className="text-2xl font-bold text-navy-deep mb-2">Check Your Inbox</h2>
        <p className="text-navy-deep/60 mb-2">
          We've sent a password reset link to
        </p>
        <p className="font-semibold text-navy-deep mb-6 text-lg">
          {email}
        </p>

        <div className="bg-gradient-to-r from-violet-50 to-violet-100 rounded-xl p-4 mb-8 border border-oxford/10 text-left">
          <p className="text-sm text-navy-deep/70 leading-relaxed">
            <span className="font-medium text-navy-deep">Didn't receive it?</span>{" "}
            Check your spam folder, or{" "}
            <button
              type="button"
              onClick={() => setViewState("form")}
              className="text-oxford font-medium hover:underline inline"
            >
              try again
            </button>{" "}
        ...
        <button
          ...
          className="inline-flex items-center gap-2 text-oxford font-semibold hover:underline transition-colors group"
        >
  ...
  return (
    <>
      <button
        ...
        className="inline-flex items-center gap-2 text-navy-deep/60 hover:text-navy-deep text-sm mb-6 transition-colors group"
      >
      ...
      <div className="mb-8">
        <h2 className="text-2xl font-bold text-navy-deep">Reset Password</h2>
        <p className="text-navy-deep/60 mt-1">Enter your email to receive a reset link</p>
      </div>

      <form onSubmit={handleSubmit} className="space-y-5">
        <div className="space-y-2">
          <Label htmlFor="reset-email" className="text-navy-deep/80 font-medium">
            Email Address
          </Label>
          <Input
            id="reset-email"
            ...
            required
            className="bg-alice-gray border-navy-deep/10 text-navy-deep placeholder:text-navy-deep/40 h-12 rounded-xl"
          />
        </div>
